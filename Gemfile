source 'https://rubygems.org'
ruby '1.9.3'

gem 'rails', '4.0.3'              # Rails                     
gem 'sass-rails', '~> 4.0.0'      # Use SCSS for stylesheets  
gem 'uglifier', '>= 1.3.0'        # JS Compressor             
gem 'coffee-rails', '~> 4.0.0'    # CoffeeScript              
gem 'jquery-rails'                # jQuery                    
gem 'turbolinks'                  # Turbolinks                https://github.com/rails/turbolinks
gem 'jbuilder', '~> 1.2'          # Build JSON APIs           https://github.com/rails/jbuilder
gem 'sorcery'                     # Authentication            https://github.com/NoamB/sorcery
gem 'formtastic'                  # Forms                     https://github.com/justinfrench/formtastic
gem 'rails_12factor'              # Environments              https://github.com/heroku/rails_12factor

group :development do
  gem 'sqlite3'                   # Development database      https://github.com/sparklemotion/sqlite3-ruby
  gem 'annotate'                  # Annotate                  https://github.com/ctran/annotate_models
  gem 'meta_request'              # Rails panel               https://github.com/dejan/rails_panel
  gem 'debugger'                  # Debug                     https://github.com/cldwalker/debugger
end

group :production do
  gem 'pg'                        # Production database       https://bitbucket.org/ged/ruby-pg/src
end